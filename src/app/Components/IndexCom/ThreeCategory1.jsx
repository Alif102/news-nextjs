...
const ThreeCategory1 = () => {
  ...
  const [threeCategoryOne, setthreeCategoryOne] = useState(null);
  ...
  useEffect(() => {
    ...
    axios.get('https://admin.desh365.top/api/structure') // Replace with your structure API URL
      .then((response) => {
        ...
        setthreeCategoryOne(response.data.structure.three_category_1);
      })
      .catch((error) => {
  ...
  const filteredData = data.filter(post => post.category_id == threeCategoryOne);
