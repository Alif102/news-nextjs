...
const FourthCategory = () => {
  const [fourthCategoryPosts, setfourthCategoryPosts] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const fourthCategory = parseInt(structureResponse.data.structure.fourth_category);
        ...
        setfourthCategoryPosts(filteredPosts);
      } catch (error) {
  ...
  return (
    <div className='grid lg:grid-cols-8 gap-2'>
      <div className='lg:col-span-4 col-span-1 flex justify-center items-center'>
        {fourthCategoryPosts.length > 0 && (
          <Link href={`/Pages/post/${fourthCategoryPosts[0]?.id}`} key={fourthCategoryPosts[0]?.id}>
            <div key={fourthCategoryPosts[0]?.id}>
              <div className='relative' style={{ height: '380px', width: '103%' }}>
                <Image
                  className='rounded-xl'
                  src={`https://admin.desh365.top/public/storage/post-image/${fourthCategoryPosts[0]?.image}`}
                  alt={fourthCategoryPosts[0]?.title || 'Default Alt Text'}
                  layout='fill'
                  objectFit='cover'
                  priority={true}
                />
      ...
      <div className='lg:col-span-4 col-span-1'>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 px-8 md:px-2 lg:px-0 py-4">
          {fourthCategoryPosts.slice(1).map(post => {
